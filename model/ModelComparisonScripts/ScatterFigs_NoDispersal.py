...
def figplot(dat, y, x, seed, xlab, ylab, fig, n):
    ...
    if n == 1:
        plt.text(-2, 3, 'Bray-Curtis', fontsize=fs+2, rotation=90)
    elif n == 4:
        plt.text(-2, 400, 'Sorensen', fontsize=fs+2, rotation=90)
    elif n == 7:
        plt.text(-2, 100, 'Canberra', fontsize=fs+2, rotation=90)
...
def figfunction(fig, met1, label, n, df):
    ...
    fig = figplot(df['fit'], y, df['env_r'], df['env_r'], xlab, ylab, fig, n)

    xlab = 'Dormant death'
    fig = figplot(df['fit'], y, df['dded'], df['env_r'], xlab, ylab, fig, n+1)
...
for label in labels:
    for met1 in metrics:  
        ...
        plt.savefig(mydir+'/figs/FromSims/temp/2x3'+label+'.png', dpi=400, bbox_inches = "tight")
        plt.close()